...
import { isBeliefPredicate } from "./predicates";

const MAX_BELIEFS = 120;

export function addBelief(npc: NpcState, belief: Belief): NpcState {
  if (!isBeliefPredicate(belief.predicate)) {
    throw new Error(`Invalid belief predicate: ${belief.predicate}`);
  }
...
export { isBeliefPredicate } from "./predicates";
export type { BeliefPredicate } from "./predicates";
